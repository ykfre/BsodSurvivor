 ...
 *
 * File is a multi-stream file with various different data streams. Some streams
 * are located at a fixed index:
...
struct pdb_link_info_header {
  ...
  uint32_t
      out_file_begin_in_command; // Example: 18 (index of 'foo.exe' in command)
...
blink::pdb_reader::pdb_reader(const std::string &path) : msf_reader(path) {
  // PDB files should have 4 streams at the beginning that are always at the
  // same index
  ...
  for (uint32_t i = 0; i < hash_table_size; i++) {
    ...
    pdb_stream.seek(
        sizeof(header) +
        name_offset); // Seek into the string table that stores the name
    const std::string name(pdb_stream.read_string());
    pdb_stream.seek(pos); // Seek previous position in stream to read next name
                          // offset in the next iteration of this loop

    _named_streams.insert({name, stream_index});
  }
}

void blink::pdb_reader::read_symbol_table(
    uint8_t *image_base, std::unordered_map<std::string, Symbol> &symbols) {
  ...
  // Find debug header stream
  // (https://llvm.org/docs/PDB/DbiStream.html#optional-debug-header-stream)
  stream.skip(header.module_info_size + header.section_contribution_size +
              header.section_map_size + header.file_info_size +
              header.ts_map_size + header.ec_info_size);
  const pdb_dbi_debug_header &debug_header =
      stream.read<pdb_dbi_debug_header>();

  // Read section headers
  stream_reader section_stream(msf_reader::stream(debug_header.section_header));

  const size_t num_sections =
      section_stream.size() / sizeof(pdb_dbi_section_header);
  const pdb_dbi_section_header *sections =
      section_stream.data<pdb_dbi_section_header>();
  ...
  parse_code_view_records(
      ...
      [&](uint16_t tag) {
        ...
        if (sym.section == 0 || sym.section > num_sections) {
          ...
          auto section = sections[sym.section - 1];
          symbols[sym.name] =
              Symbol(sym.name,
                     image_base + sections[sym.section - 1].virtual_address +
                         sym.offset,
                     section.flags & IMAGE_SCN_MEM_WRITE ? Symbol::IsConst::NOT
                                                         : Symbol::IsConst::IS,
                     Symbol::IsFromExport::NOT,
                     ISFCN(sym.flags) ? Symbol::IsFunction::IS
                                      : Symbol::IsFunction::NOT);
        }
      },
      4);
}

void blink::pdb_reader::read_object_files(
  ...
  // Read module information stream
  ...
  while (stream.tell() < sizeof(pdb_dbi_header) + header.module_info_size) {
    ...
    const auto obj_file_name =
        stream.read_string(); // Contains the name of the ".lib" if this object
                              // file is part of a library
    ...
    if (path.is_relative()) {
      if (info.symbol_stream != 65535 /*-1*/) {
        ...
        // Look up current working directory in symbol stream
        // https://llvm.org/docs/PDB/ModiStream.html
        stream_reader stream2(msf_reader::stream(info.symbol_stream));
        stream2.skip(4); // Skip 32-bit signature (this should be
                         // CV_SIGNATURE_C13, aka 4)

        parse_code_view_records(
            stream2, info.symbol_byte_size - 4, [&](uint16_t tag) {
              if (tag == 0x113d) // S_ENVBLOCK
              {
                stream2.skip(1);
                while (stream2.tell() < stream2.size() &&
                       *stream2.data() != '\0') {
                  const auto key = stream2.read_string();
                  const std::string value(stream2.read_string());
...
void blink::pdb_reader::read_source_files(
  ...
  stream.skip(2); // Skip old number of file names (see comment on counting the
                  // number below)

  const uint16_t *const module_file_offsets = stream.data<uint16_t>();
  const uint16_t *const module_num_source_files =
      stream.data<uint16_t>(num_modules * sizeof(uint16_t));
  const uint32_t *const file_name_offsets =
      stream.data<uint32_t>(num_modules * sizeof(uint16_t) * 2);

  // Count number of source files instead of reading the value from the header,
  // since there may be more source files that would fit into a 16-bit value
  uint32_t num_source_files = 0;
  for (uint16_t i = 0; i < num_modules; ++i)
    num_source_files += module_num_source_files[i];

  stream.skip(num_modules * sizeof(uint16_t) * 2 +
              num_source_files * sizeof(uint32_t));
...
void blink::pdb_reader::read_link_info(std::filesystem::path &cwd,
                                       std::string &cmd) {
  ...
  // See
  // https://github.com/Microsoft/microsoft-pdb/blob/master/langapi/include/pdb.h#L500
...
void blink::pdb_reader::read_name_hash_table(
  ...
  for (uint32_t i = 0; i < size; i++) {
    ...
    stream.seek(pos); // Seek previous position in stream to read next name
                      // offset in the next iteration of this loop
