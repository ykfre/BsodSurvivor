...
// workaround for missing traits in GCC and CLANG
        ...
        {
            const static bool value = std::is_nothrow_constructible<T, T&&>::value;
        };
        ...
        struct is_assignable
        {
            ...
            const static bool has_assign(...) { return false; }

            template <class X, class Y, size_t S = sizeof((std::declval<X>() = std::declval<Y>(), true)) >
            // the comma operator is necessary for the cases where operator= returns void
            const static bool has_assign(bool) { return true; }

            const static bool value = has_assign<T, U>(true);
        };
        ...
        struct is_nothrow_move_assignable
        {
            ...
            struct has_nothrow_move_assign<X, true> {
                const static bool value = noexcept(std::declval<X&>() = std::declval<X&&>());
            };

            const static bool value = has_nothrow_move_assign<T, is_assignable<T&, T&&>::value>::value;
        };
        // end workaround
...
# define TR2_OPTIONAL_ASSERTED_EXPRESSION(CHECK, EXPR) ((CHECK) ? (EXPR) : ([]{assert(!#CHECK);}(), (EXPR)))
        ...
        namespace detail_
        {
            ...
            {
                template <class X>
                const static bool has_overload(...) { return false; }

                template <class X, size_t S = sizeof(std::declval<X&>().operator&()) >
                const static bool has_overload(bool) { return true; }
